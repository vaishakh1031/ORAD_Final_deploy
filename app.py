...
# ---------- CLIP model loading DISABLED for Railway deployment ----------
...
# ---------- CLIP validation function DISABLED ----------
def is_ovarian_ultrasound(image_path, threshold=0.5):
    """
    DISABLED – Always returns True to avoid model loading.
    In production, you can re‑enable CLIP when more memory is available.
    """
    print("CLIP validation is disabled for deployment. Accepting any image.")
    return True
...
TRANSLATIONS = {
    ...
    "hi": {
        ...
        "reasoning_low": "यह घाव {size_mm} मिमी है, {cyst_type} प्रकार केवल {solid_components_percentage}% ठोस घटकों के साथ। दीवार {wall_irregularity} है और संवहनीयता {vascularity} है। कोई चिंताजनक विभाजन नहीं। ये सभी सौम्य विशेषताएं हैं।",
        ...
        "reasoning_high": "महत्वपूर्ण निष्कर्ष: आकार {size_mm} मिमी, {cyst_type} पुटी {solid_components_percentage}% ठोस घटकों के साथ। संवहनीयता {vascularity} है, और दीवार {wall_irregularity} है। विभाजन: {septations}. ये O-RADS {orad_score} से जुड़ी अत्यधिक संदिग्ध विशेषताएं हैं और O-RADS 5 के लिए घातकता का जोखिम >50%, O-RADS 4 के लिए 5-50% है।",
...
def predict():
    ...
    try:
        # ---------- CLIP validation is now disabled (always returns True) ----------
        # The function is_ovarian_ultrasound still exists but always returns True.
        # You can remove the check entirely if you prefer, but we keep it for structure.
        ...
        if orad <= 2:
            reasoning = get_localized_text(lang, "reasoning_low",
            ...
            reasoning = get_localized_text(lang, "reasoning_medium",
                size_mm=clinical['size_mm'],
                cyst_type=clinical['cyst_type'],
                solid_components_percentage=clinical['solid_components_percentage'],
                vascularity=clinical['vascularity'],
            ...
            reasoning = get_localized_text(lang, "reasoning_high",
                size_mm=clinical['size_mm'],
                cyst_type=clinical['cyst_type'],
                solid_components_percentage=clinical['solid_components_percentage'],
                vascularity=clinical['vascularity'],
